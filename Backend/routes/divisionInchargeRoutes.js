...
router.get("/me", authenticateToken, authorizeRoles("divisionIncharge"),getMyProfile);

// get single division incharge by ID(admin on clicking a Division Incharge card can see all details)
router.get("/:id", authenticateToken, authorizeRoles("admin"), getSingleDivisionInchargeById);


// Exisiting Division Incharge's name, div and year can be changed through this route
router.patch("/:id", authenticateToken, authorizeRoles("admin"), changeDivisionInchargeDetails);
...
router.patch("/change-email/:id", authenticateToken, authorizeRoles("admin"), changeEmailOfDivisionIncharge);
