...
const {
  internshipValidationSchema,
  updateInternshipValidationSchema,
  getInternshipsValidation,
} = require("../validators/internshipValidation");
...
const createInternship = async (req, res) => {
  ...
  try {
    ...
    const { error } = internshipValidationSchema.validate(
      {
        companyName,
        startDate,
        endDate,
    ...
    if (
      parsedIsPaid &&
      (parsedStipend === undefined || parsedStipend === null)
    ) {
      return res.status(400).json({
        success: false,
    ...
    if (!internshipReport || !photoProof) {
      return res.status(400).json({
        success: false,
        message: "Both internship report and photo proof are required",
      });
    }
    ...
    if (internshipReport.mimetype !== ALLOWED_REPORT_TYPE) {
      return res
        .status(400)
        .json({ success: false, message: "Internship report must be a PDF" });
    }

    if (internshipReport.size > MAX_FILE_SIZE) {
      return res
        .status(400)
        .json({ success: false, message: "Internship report exceeds 5MB" });
    }

    if (!ALLOWED_PROOF_TYPES.includes(photoProof.mimetype)) {
      return res
        .status(400)
        .json({ success: false, message: "Photo proof must be JPG or PNG" });
    }

    if (photoProof.size > MAX_FILE_SIZE) {
      return res
        .status(400)
        .json({ success: false, message: "Photo proof exceeds 5MB" });
    }

    ({ reportResult, proofResult } = await uploadInternshipFiles(
      internshipReport.path,
      photoProof.path
    ));
    ...
    if (!dbSaved) {
      if (reportResult?.publicId)
        await cloudinary.uploader.destroy(reportResult.publicId);
      if (proofResult?.publicId)
        await cloudinary.uploader.destroy(proofResult.publicId);
    }
    res.status(500).json({
      success: false,
...
const getOwnInternships = async (req, res) => {
  try {
    ...
    const internships = await Internship.find({ stuID: studentId }).sort({
      startDate: -1,
    });
  ...
  try {
    ...
    if (req.user.role === "student") {
      ...
      if (existingInternship.stuID.toString() !== userId.toString()) {
        return res.status(400).json({
          success: false,
          message: "Internship does not belong to the logged in student",
        });
      }
    }
    ...
    if (internshipReport) {
      ...
      if (existingInternship.internshipReport?.publicId) {
        await cloudinary.uploader.destroy(
          existingInternship.internshipReport.publicId
        );
      }
    ...
    if (photoProof) {
      ...
      if (existingInternship.photoProof?.publicId) {
        await cloudinary.uploader.destroy(
          existingInternship.photoProof.publicId
        );
      }
    ...
    if (!dbSaved) {
      if (newReportPublicId)
        await cloudinary.uploader.destroy(newReportPublicId);
  ...
  try {
    ...
    const getInternships = async (req, res) => {
      try {
        ...
        // Fetch all internships with student info populated
        const internships = await Internship.find()
          .populate({
            path: "stuID",
            select: "name studentID branch year",
          })
          .sort({ startDate: -1 });
